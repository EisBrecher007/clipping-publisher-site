...
export default { async fetch(request, env) {
  ...
  try {
    ...
    if (request.method === "POST" && url.pathname === "/api/token-refresh-check") { const { sid, token } = await tokenFor(request, env); const refreshed = await refreshToken(sid, token, env); return json({ refreshed: true, scopes: refreshed.scope || "" }, 200, headers); }
